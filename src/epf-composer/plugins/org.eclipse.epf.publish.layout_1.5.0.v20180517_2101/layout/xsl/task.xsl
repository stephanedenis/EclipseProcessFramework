...
<xsl:output method="html" version="1.0" encoding="UTF-8" 
	...
	<xsl:template name="relationshipsSection">
		...
		<xsl:variable name="oppositeLists" select="referenceList[@referenceType='customOpposite']"/>
		
		<xsl:if test="count($practices) + count($performedBy) + count($categories) + count($additionallyPerformedBy) + count($workProductMandatoryInputs) + count($workProductSlotMandatoryInputs) + count($workProductOptionalInputs) + count($workProductSlotOptionalInputs) + count($workProductOutputs) + count($workProductSlotOutputs) + count($oppositeLists) > 0">
